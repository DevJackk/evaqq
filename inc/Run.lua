...
eva = Token:match("(%d+)")
redis:set(eva..":VERSION",1)
redis:set(eva..":SUDO_ID:",SUDO_USER)
redis:set(eva..":DataCenter:",'German')
redis:set(eva..":UserNameBot:",BOT_User)
redis:set(eva..":NameBot:",BOT_NAME)
redis:hset(eva..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3eva_INSTALL","Yes")
info = {}
...
Bot_User = redis:get(eva..":UserNameBot:")
SUDO_ID = tonumber(redis:get(eva..":SUDO_ID:"))
SUDO_USER = redis:hgetall(eva..'username:'..SUDO_ID).username
version = redis:get(eva..":VERSION")
DataCenter = redis:get(eva..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..eva..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(eva..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(eva..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(eva..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(eva..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(eva..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(eva..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(eva..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(eva..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(eva..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(eva..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(eva..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(eva..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(eva..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(eva..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(eva..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس ايفا ⇐ @SOURCE_eva
...
if msg.GroupActive and redis:get(eva..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(eva..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(eva..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(eva..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.eva then 
if msg.text and ScriptFile.ieva then
for k, eva in pairs(ScriptFile.eva) do
local SearchText = msg.text:match(eva)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ieva(msg,{msg.text:match(eva)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",eva.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.deva then
if ScriptFile.deva(msg) == false then
return false
end
...
if Plug.eva then 
if msg.text and Plug.ieva then
for k, eva in pairs(Plug.eva) do
local SearchText = msg.text:match(eva)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(eva..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(eva..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ieva(msg,{msg.text:match(eva)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",eva.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source eva")
end 

end
else
print("The File Script.lua Not Runing in The Source eva")
...
function tdcli_update_callback(data)
	...
	redis:srem(eva..'users',msg.sender_user_id_)
	end
	...
	EditMsg(data.message_.chat_id_,data.message_.id_,'10% - E')
	EditMsg(data.message_.chat_id_,data.message_.id_,'20% - E')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/eva/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - EV')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/eva/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - EV')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/eva/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - EVA  ')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/eva/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/eva/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/eva/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - |eva|\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل السورس بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(eva..":SUDO_ID:")
redis:hset(eva..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(eva..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(eva..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(eva..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(eva..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(eva..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(eva..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(eva..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(eva..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(eva..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(eva..'group:add'..data.chat_.id_) then
	redis:set(eva..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(eva..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(eva..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(eva..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	eva..'group:name'..k,IDS.Title,
	eva..'num_msg_eva'..k,5,
	eva..'group:add'..k,true,
	eva..'lock_link'..k,true,
	eva..'lock_spam'..k,true,
	eva..'lock_webpage'..k,true,
	eva..'lock_markdown'..k,true,
	eva..'lock_flood'..k,true,
	eva..'lock_bots'..k,true,
	eva..'mute_forward'..k,true,
	eva..'mute_contact'..k,true,
	eva..'mute_document'..k,true,
	eva..'mute_inline'..k,true,
	eva..'lock_username'..k,true,
	eva..'replay'..k,true
	)
	redis:sadd(eva..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(eva..'username:'..ID,'username',user)
	redis:sadd(eva..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(eva..'username:'..ID,'username',user)
	redis:sadd(eva..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(eva..'username:'..ID,'username',user)
	redis:sadd(eva..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(eva..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(eva..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(eva..'username:'..data.user_.id_).username
	...
	redis:hset(eva..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
